//! SkyClaw Skills crate
//!
//! Provides skill discovery, parsing, and registry for the SkyHub v1 skill system.
...
impl SkillRegistry {
    ...
    /// Scan the global (`~/.skyclaw/skills/`) and workspace (`<workspace>/skills/`)
    ...
    pub async fn load_skills(&mut self) -> Result<(), SkyclawError> {
        self.skills.clear();
        ...
        if let Some(home) = dirs::home_dir() {
            let global_dir = home.join(".skyclaw").join("skills");
...
async fn scan_directory(dir: &Path) -> Result<Vec<Skill>, SkyclawError> {
    let mut loaded = Vec::new();
    let mut entries = fs::read_dir(dir).await.map_err(|e| {
        SkyclawError::Skill(format!("Failed to read directory {}: {}", dir.display(), e))
    })?;

    while let Some(entry) = entries.next_entry().await.map_err(|e| {
        SkyclawError::Skill(format!("Failed to read entry in {}: {}", dir.display(), e))
    })? {
        let path = entry.path();
...
async fn parse_skill_file(path: &Path) -> Result<Skill, SkyclawError> {
    let content = fs::read_to_string(path)
        .await
        .map_err(|e| SkyclawError::Skill(format!("Failed to read {}: {}", path.display(), e)))?;

    parse_skill_content(&content, path)
}

/// Parse skill content (frontmatter + body) from a string.
fn parse_skill_content(content: &str, source_path: &Path) -> Result<Skill, SkyclawError> {
    let trimmed = content.trim_start();

    if !trimmed.starts_with("---") {
        return Err(SkyclawError::Skill(format!(
