...
impl GitTool {
    ...
    fn build_args(action: &str, args: &serde_json::Value) -> Result<Vec<String>, SkyclawError> {
        ...
        match action {
            "clone" => {
                let url = args
                    .get("url")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| SkyclawError::Tool("clone requires 'url' parameter".into()))?;
            ...
            "commit" => {
                let message = args
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        SkyclawError::Tool("commit requires 'message' parameter".into())
                    })?;
            ...
            "checkout" => {
                let branch = args.get("branch").and_then(|v| v.as_str()).ok_or_else(|| {
                    SkyclawError::Tool("checkout requires 'branch' parameter".into())
                })?;
            ...
            "add" => {
                ...
                if all {
                    ...
                    let files = args
                        ...
                        .ok_or_else(|| {
                            SkyclawError::Tool("add requires 'files' array or 'all: true'".into())
                        })?;
    ...
    fn validate_safety(action: &str, args: &serde_json::Value) -> Result<(), SkyclawError> {
