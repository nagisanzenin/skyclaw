...
impl TaskStatus {
    ...
    fn from_str(s: &str) -> Result<Self, SkyclawError> {
        match s {
            "pending" => Ok(Self::Pending),
            "running" => Ok(Self::Running),
            "completed" => Ok(Self::Completed),
            "failed" => Ok(Self::Failed),
            other => Err(SkyclawError::Internal(format!(
...
impl TaskQueue {
    ...
    pub async fn new(database_url: &str) -> Result<Self, SkyclawError> {
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .map_err(|e| SkyclawError::Internal(format!("TaskQueue connect failed: {e}")))?;
    ...
    async fn init_tables(&self) -> Result<(), SkyclawError> {
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS skyclaw_tasks (
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| SkyclawError::Internal(format!("TaskQueue create table failed: {e}")))?;

        sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_status ON skyclaw_tasks(status)")
            .execute(&self.pool)
            .await
            .map_err(|e| SkyclawError::Internal(format!("TaskQueue create index failed: {e}")))?;

        sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_chat ON skyclaw_tasks(chat_id)")
            .execute(&self.pool)
            .await
            .map_err(|e| SkyclawError::Internal(format!("TaskQueue create index failed: {e}")))?;
    ...
    pub async fn create_task(&self, chat_id: &str, goal: &str) -> Result<String, SkyclawError> {
        let task_id = Uuid::new_v4().to_string();
        let now = Utc::now().to_rfc3339();

        sqlx::query(
            r#"
            INSERT INTO skyclaw_tasks (task_id, chat_id, goal, status, created_at, updated_at)
            VALUES (?, ?, ?, 'pending', ?, ?)
            "#,
        )
        .bind(&task_id)
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| SkyclawError::Internal(format!("TaskQueue create failed: {e}")))?;
    ...
    pub async fn update_status(
        ...
        sqlx::query("UPDATE skyclaw_tasks SET status = ?, updated_at = ? WHERE task_id = ?")
            .bind(status.as_str())
            .bind(&now)
            .bind(task_id)
            .execute(&self.pool)
            .await
            .map_err(|e| SkyclawError::Internal(format!("TaskQueue update_status failed: {e}")))?;
    ...
    pub async fn checkpoint(&self, task_id: &str, session_data: &str) -> Result<(), SkyclawError> {
        let now = Utc::now().to_rfc3339();

        sqlx::query(
            "UPDATE skyclaw_tasks SET checkpoint_data = ?, updated_at = ? WHERE task_id = ?",
        )
        .bind(session_data)
        .bind(&now)
        .bind(task_id)
        .execute(&self.pool)
        .await
        .map_err(|e| SkyclawError::Internal(format!("TaskQueue checkpoint failed: {e}")))?;
    ...
    pub async fn load_incomplete(&self) -> Result<Vec<TaskEntry>, SkyclawError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| SkyclawError::Internal(format!("TaskQueue load_incomplete failed: {e}")))?;

        rows.into_iter().map(row_to_entry).collect()
    }

    /// List all tasks, optionally filtered by chat_id.
    pub async fn list_tasks(&self, chat_id: Option<&str>) -> Result<Vec<TaskEntry>, SkyclawError> {
        ...
        .map_err(|e| SkyclawError::Internal(format!("TaskQueue list_tasks failed: {e}")))?;
...
fn row_to_entry(row: TaskRow) -> Result<TaskEntry, SkyclawError> {
    let status = TaskStatus::from_str(&row.status)?;
    let created_at = chrono::DateTime::parse_from_rfc3339(&row.created_at)
        .map_err(|e| SkyclawError::Internal(format!("Invalid created_at: {e}")))?
        .with_timezone(&Utc);
    let updated_at = chrono::DateTime::parse_from_rfc3339(&row.updated_at)
        .map_err(|e| SkyclawError::Internal(format!("Invalid updated_at: {e}")))?
        .with_timezone(&Utc);
