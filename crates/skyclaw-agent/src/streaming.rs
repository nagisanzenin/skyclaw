...
impl StreamingNotifier {
    ...
    pub async fn notify_tool_start(&self, tool_name: &str) -> Result<(), SkyclawError> {
    ...
    pub async fn notify_status(&self, message: &str) -> Result<(), SkyclawError> {
        if !self.enabled {
            return Ok(());
        }
    ...
    async fn send_status(&self, text: &str) -> Result<(), SkyclawError> {
    ...
    impl Channel for MockChannel {
        ...
        async fn send_message(&self, msg: OutboundMessage) -> Result<(), SkyclawError> {
            if let Some(ref err) = self.fail_with {
                return Err(SkyclawError::Channel(err.clone()));
            }
