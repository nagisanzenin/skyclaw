//! Startup metrics and health verification for SkyClaw.
//!
...
//! ```bash
//! cargo build --release --bin skyclaw --no-default-features
//! ```
...
///
/// Returns `Ok(())` if all checks pass, or `Err(SkyclawError::Internal)`
/// with a description of what failed.
pub fn check_startup_health(metrics: &StartupMetrics) -> Result<(), SkyclawError> {
...
mod tests {
    ...
    async fn lazy_resource_initializes_once() {
        ...
        let val = resource
            .get_or_init(|| async { Ok::<_, SkyclawError>("initialized".to_string()) })
            .await
            .unwrap();
        ...
        let val2 = resource
            .get_or_init(|| async { Ok::<_, SkyclawError>("should not run".to_string()) })
            .await
            .unwrap();
    ...
    async fn lazy_resource_retries_on_failure() {
        ...
        let err = resource
            .get_or_init(|| async {
                Err::<String, SkyclawError>(SkyclawError::Internal("boom".to_string()))
            })
            .await;
        assert!(err.is_err());
        assert!(!resource.is_initialized());
        ...
        let val = resource
            .get_or_init(|| async { Ok::<_, SkyclawError>("recovered".to_string()) })
            .await
            .unwrap();
