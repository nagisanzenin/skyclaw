...
impl PromptPatchManager {
    ...
    pub fn propose_patch(&mut self, mut patch: PromptPatch) -> Result<String, SkyclawError> {
    ...
    pub fn approve_patch(&mut self, id: &str) -> Result<(), SkyclawError> {
        let patch = self
            .patches
            .iter_mut()
            .find(|p| p.id == id)
            .ok_or_else(|| SkyclawError::NotFound(format!("Prompt patch not found: {id}")))?;
    ...
    pub fn reject_patch(&mut self, id: &str) -> Result<(), SkyclawError> {
        let patch = self
            .patches
            .iter_mut()
            .find(|p| p.id == id)
