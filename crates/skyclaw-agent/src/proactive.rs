...
impl ProactiveManager {
    ...
    pub fn add_rule(&mut self, rule: TriggerRule) -> Result<String, SkyclawError> {
    ...
    pub fn remove_rule(&mut self, rule_id: &str) -> Result<(), SkyclawError> {
        let before = self.rules.len();
        self.rules.retain(|r| r.id != rule_id);
        if self.rules.len() == before {
            return Err(SkyclawError::NotFound(format!(
