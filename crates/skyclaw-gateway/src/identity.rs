...
use skyclaw_core::{AuthResult, Identity};
...
impl OAuthIdentityManager {
    ...
    pub async fn start_oauth_flow(
        ...
        let config = self
            ...
            .ok_or_else(|| SkyclawError::Auth(format!("Unknown OAuth provider: {}", provider)))?
            .clone();
    ...
    pub async fn complete_oauth_flow(
        ...
        state: &str,
        code: &str,
    ) -> Result<OAuthToken, SkyclawError> {
        // Look up and remove the pending flow
        let flow = {
            let mut flows = self.pending_flows.write().await;
            flows.remove(state).ok_or_else(|| {
                SkyclawError::Auth("Invalid or expired OAuth state parameter".to_string())
            })?
        };
        ...
        let config = self
            ...
            .ok_or_else(|| {
                SkyclawError::Auth(format!("Provider config not found: {}", flow.provider))
            })?
            .clone();
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| SkyclawError::Auth(format!("Token exchange HTTP error: {}", e)))?;
    ...
    pub async fn refresh_token(
        ...
        let (refresh_tok, config) = {
            ...
            let user = users
                .get(user_id)
                .ok_or_else(|| SkyclawError::Auth(format!("User not found: {}", user_id)))?;
            ...
            let refresh = current
                .refresh_token
                .clone()
                .ok_or_else(|| SkyclawError::Auth("No refresh token available".to_string()))?;
            let cfg = self
                .find_provider_config(provider)
                .ok_or_else(|| SkyclawError::Auth(format!("Unknown OAuth provider: {}", provider)))?
                .clone();
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| SkyclawError::Auth(format!("Token refresh HTTP error: {}", e)))?;
...
impl Identity for OAuthIdentityManager {
    ...
    async fn has_permission(&self, user_id: &str, permission: &str) -> Result<bool, SkyclawError> {
        ...
        match users.get(user_id) {
            ...
            None => Err(SkyclawError::Auth(format!("User not found: {}", user_id))),
        }
    }
