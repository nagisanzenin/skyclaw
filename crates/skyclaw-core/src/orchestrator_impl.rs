...
pub trait DockerClient: Send + Sync {
    ...
    async fn start_container(&self, id: &str) -> Result<(), SkyclawError>;

    /// POST /containers/{id}/stop
    async fn stop_container(&self, id: &str) -> Result<(), SkyclawError>;

    /// DELETE /containers/{id}?force=true
    async fn remove_container(&self, id: &str) -> Result<(), SkyclawError>;

    /// GET /containers/{id}/json — returns the container status string.
    async fn inspect_container(&self, id: &str) -> Result<ContainerInspect, SkyclawError>;
}
...
impl Orchestrator for DockerOrchestrator {
    ...
    async fn scale(&self, instance: &AgentInstance, replicas: u32) -> Result<(), SkyclawError> {
        if replicas == 0 {
            return Err(SkyclawError::Internal(
...
impl Orchestrator for KubernetesOrchestrator {
    async fn provision(&self, _spec: AgentSpec) -> Result<AgentInstance, SkyclawError> {
        Err(SkyclawError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn scale(&self, _instance: &AgentInstance, _replicas: u32) -> Result<(), SkyclawError> {
        Err(SkyclawError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn destroy(&self, _instance: &AgentInstance) -> Result<(), SkyclawError> {
        Err(SkyclawError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn health(&self, _instance: &AgentInstance) -> Result<bool, SkyclawError> {
        Err(SkyclawError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }
...
) -> Result<Box<dyn Orchestrator>, SkyclawError> {
    match backend {
        "docker" => {
...
impl DockerClient for UnimplementedDockerClient {
    ...
    async fn start_container(&self, _id: &str) -> Result<(), SkyclawError> {
        Err(SkyclawError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn stop_container(&self, _id: &str) -> Result<(), SkyclawError> {
        Err(SkyclawError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn remove_container(&self, _id: &str) -> Result<(), SkyclawError> {
        Err(SkyclawError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn inspect_container(&self, _id: &str) -> Result<ContainerInspect, SkyclawError> {
        Err(SkyclawError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }
...
mod tests {
    ...
    impl DockerClient for MockDockerClient {
        async fn create_container(
            ...
            if self.fail_create {
                return Err(SkyclawError::Internal("Docker create failed".to_string()));
            }
        ...
        async fn stop_container(&self, _id: &str) -> Result<(), SkyclawError> {
            if self.fail_stop {
                return Err(SkyclawError::Internal("Docker stop failed".to_string()));
            }
            Ok(())
        }
