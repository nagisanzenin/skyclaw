//! Test utilities and mocks for SkyClaw.
//!
...
/// Fluent builder for `SkyclawConfig` in tests.
pub struct TestConfigBuilder {
    config: SkyclawConfig,
}

impl TestConfigBuilder {
    pub fn new() -> Self {
        Self {
            config: SkyclawConfig::default(),
        }
    }
    ...
    pub fn build(self) -> SkyclawConfig {
        self.config
    }
}
...
pub fn make_session() -> SessionContext {
    SessionContext {
        ...
        user_id: "test-user".to_string(),
        history: Vec::new(),
        workspace_path: std::env::temp_dir().join("skyclaw-test"),
    }
}
