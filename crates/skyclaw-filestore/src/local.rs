...
impl LocalFileStore {
    ...
    fn resolve_path(&self, path: &str) -> Result<PathBuf, SkyclawError> {
...
impl FileStore for LocalFileStore {
    async fn store(
        ...
        fs::write(&full_path, &data).await.map_err(|e| {
            SkyclawError::FileTransfer(format!("Failed to write file {}: {e}", full_path.display()))
        })?;
    ...
    async fn get(&self, key: &str) -> Result<Option<Bytes>, SkyclawError> {
        ...
        let data = fs::read(&full_path).await.map_err(|e| {
            SkyclawError::FileTransfer(format!("Failed to read file {}: {e}", full_path.display()))
        })?;
...
impl LocalFileStore {
    ...
    async fn list_recursive(
        ...
        let mut entries = fs::read_dir(dir).await.map_err(|e| {
            SkyclawError::FileTransfer(format!("Failed to read directory {}: {e}", dir.display()))
        })?;

        while let Some(entry) = entries.next_entry().await.map_err(|e| {
            SkyclawError::FileTransfer(format!("Failed to read directory entry: {e}"))
        })? {
            let path = entry.path();
            if path.is_dir() {
