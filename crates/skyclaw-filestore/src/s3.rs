...
use skyclaw_core::file::FileMetadata;
use skyclaw_core::types::config::FileStoreConfig;
use skyclaw_core::FileStore;
...
impl S3FileStore {
    ...
    pub async fn new(config: &FileStoreConfig) -> Result<Self, SkyclawError> {
        let bucket = config.bucket.clone().ok_or_else(|| {
            SkyclawError::FileTransfer("S3 file store requires a 'bucket' in config".to_string())
        })?;
...
impl FileStore for S3FileStore {
    async fn store(
        ...
        self.client
            ...
            .await
            .map_err(|e| {
                SkyclawError::FileTransfer(format!("S3 PutObject failed for {path}: {e}"))
            })?;
    ...
    async fn store_stream(
        ...
        let create = self
            ...
            .await
            .map_err(|e| {
                SkyclawError::FileTransfer(format!(
                    "S3 CreateMultipartUpload failed for {path}: {e}"
                ))
            })?;

        let upload_id = create.upload_id().ok_or_else(|| {
            SkyclawError::FileTransfer("S3 CreateMultipartUpload returned no upload_id".to_string())
        })?;
    ...
    async fn presigned_url(
        ...
        let presign_config = PresigningConfig::expires_in(Duration::from_secs(expires_in_secs))
            .map_err(|e| SkyclawError::FileTransfer(format!("Invalid presigning duration: {e}")))?;
    ...
    async fn delete(&self, key: &str) -> Result<(), SkyclawError> {
        self.client
            ...
            .await
            .map_err(|e| {
                SkyclawError::FileTransfer(format!("S3 DeleteObject failed for {key}: {e}"))
            })?;
