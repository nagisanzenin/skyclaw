...
- If you state intent (\"Let me X\", \"I'll check Y\", \"checking now\", \
  \"I should X\"), you MUST emit the corresponding tool call in the SAME \
  response. Stating intent without calling the tool ends the turn — the user \
  sees your promise and an idle bot.\n\
- If you have nothing more to do, just answer the user directly. Do not \
  preface with \"let me\" if there is nothing to do.\n\
- If a tool result was incomplete (e.g. file_read showed [TRUNCATED — showing \
  lines X-Y of Z]), call the tool again with adjusted parameters in this \
  same turn. Do not promise to retry later.\n\n\
PERSISTENT MEMORY:\n\
                                                    ...
                                                    if let Some(ref agent) = agent_state.read().await.as_ref().cloned() {
                                                        ...
                                                        if let Ok(Some(order_id)) = decompose_result {
                                                            ...
                                                            let swarm_result = hive.execute_order(
                                                                ...
                                                                move |task, deps| {
                                                                    ...
                                                                    async move {
                                                                        ...
                                                                        ).with_budget(runtime_budget.clone()).with_durable_execution()
                                                            ...
                                                            let fallback_agent = Arc::new(temm1e_agent::AgentRuntime::with_limits(
                                                                ...
                                                                Some(build_system_prompt(&personality)),
                                                                max_turns, max_ctx, max_rounds, max_task_duration, max_spend,
                                                            ).with_budget(runtime_budget.clone()).with_durable_execution().with_v2_optimizations(v2_opt).with_self_audit_enabled(self_audit_opt).with_parallel_phases(pp_opt).with_shared_mode(shared_mode.clone()).with_shared_memory_strategy(shared_memory_strategy.clone()).with_personality(personality.clone()).with_social(social_storage.clone(), Some(social_config_captured.clone())).with_witness_attachments(witness_attachments.as_ref()));
