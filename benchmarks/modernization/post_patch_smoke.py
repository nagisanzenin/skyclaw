#!/usr/bin/env python3
"""Two post-dependency-patch acceptance cases; not a replacement A/B study."""
...
def main():
 ...
 args=p.parse_args();corpus=Path(__file__).with_name('closeout-corpus.json');tasks=[next(t for t in json.loads(corpus.read_text()) if t['id']=='utf8_excerpt'),json.loads(Path(__file__).with_name('closeout-clarification.json').read_text())['corrected_task']];assert len(tasks)==2
