...
Checkpoint **36** shares the owning TUI budget with TemDOS/AgentRuntime/JIT/Perpetuum and retains it through model switch/rollback. Read `SHARED-ACCOUNTING-IMPLEMENTATION.md`. The live handle owns the Arc; `spawn_agent_with_budget` receives it, while fresh sessions create a tracker. Only auxiliary Perpetuum is wrapped, not already-accounted foreground calls.

Validation: **50 TUI tests**, minimal-feature real TUI build and extended Unix PTY passed. Normal two-turn/model switch made four selected-endpoint requests; restart/history/onboarding made zero. Priced-budget case made two first-turn requests and **zero after switching an exhausted session**. Exact termios/resize/clean exit passed. Log `implementation-tui-budget-pty-v3.log`; preceding two fixture failures are retained (incorrect charged-call assumption; ANSI-separated error text). Final TUI/all-target/all-feature lint passed20.13s and cleaned1.7GiB (`implementation-tui-budget-clippy.log`). No Cargo remains active.

Previous push **35 `76350ea`**, CI34268068127, and **34 `e5dc5d9`**, CI34267201313, pending. **33 `a424b74`**, CI34265542264, **32 `816a75d`**, CI34263918328, **31 `df4e6bd`**, CI34263300717, and **30 `9df96c9`**, CI34261794031, are fully green. **29 `06d9bbd`**, CI34261489651, remains recorded Windows failure (Hive stale-readiness race repaired in32).

Next concrete task: **classifier accounting on parse failure/timeout**. `llm_classifier.rs::classify_message` receives a successful response then uses `parse_classification(...)?`, dropping usage on parse error. Runtime1293+ only records usage in the Ok arm. This explains the PTY fixture's two HTTP requests but only one charged response. Wrap classifier calls with MeteredProvider using a local BudgetTracker child, accumulate per-turn values from that child's snapshot regardless of parse result, remove existing success re-record to avoid doubling. Cancellation/timeout should preserve unknownness; admission denial should not fabricate a provider call. Add deterministic malformed-JSON and timeout/cancel fixtures through actual classifier/runtime paths. Update the priced PTY fixture when both actual calls are charged; do not weaken admission assertions. Then propagate owning root CLI/server budgets through remaining constructors and continue durable goal/evidence contracts and broad held-out A/B. Preserve D01/D02 defaults.
...
After pushing checkpoint 36, inspect CI and fix classifier lost usage above. Keep the following boundaries visible; the browser checkpoint is not complete product acceptance.
