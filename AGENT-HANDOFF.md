...
Checkpoint **40** applies a shared RuntimePolicy to all25root constructors, TUI setup/replacement and JIT/automatic Hive workers. Explicit Engram disablement and curatoroff no longer revert to defaults. Effective parent overrides and budget identity are preserved. Read `RUNTIME-POLICY-IMPLEMENTATION.md`.
...
Previous push **39 `aa250eb`**, CI34272999185 was still in progress at last check. **38 `b63fcc5`**, CI34271019124, and **37 `2d5f122`**, CI34269989662, fully green. Earlier Windows failure29 remains recorded and fixed32.
...
After pushing checkpoint 40, inspect CI and preserve auxiliary accounting work above. Keep the following boundaries visible; the browser checkpoint is not complete product acceptance.
