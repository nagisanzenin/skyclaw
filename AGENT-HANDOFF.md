...
Checkpoint **34** implements source-backed image knownness, explicit custom overrides and truthful CLI/TUI badges. Read `MODEL-CATALOG-IMPLEMENTATION.md`. Thirty-nine catalog rows have primary-source/date metadata; absent capability is unknown. Runtime snapshots provider/model/custom selection. Unknown forwarding retains the historical compatibility policy and says so; this is not complete endpoint-aware ResolvedModel or entitlement discovery.

Validation: **787 agent, 257 core (one ignored), 45 TUI and 79 root minimal-feature tests** passed. Four pre-existing cfg-dependent root unused-mut warnings remain. Final full workspace/all-feature/all-target lint passed (`implementation-image-capability-workspace-clippy.log`, 1m52s); the guard cleaned 2.6 GiB. Cargo reports a dependency future-compatibility notice for proc-macro-error2 2.0.1; no lint failure. No Cargo process remains.

Previous push **33 `a424b74`**, CI **34265542264**: Windows/Linux/MSRV/lint and both Linux builds passed, Docker pending at last inspection. **32 `816a75d`**, CI **34263918328**, **31 `df4e6bd`**, CI34263300717, and **30 `9df96c9`**, CI34261794031, are fully green. **29 `06d9bbd`**, CI34261489651, remains a recorded Windows failure (Hive stale readiness race repaired in32), not erased by an intermittent rerun.

Next concrete task: TUI `spawn_agent` currently ignores `setup.base_url` and reads the saved active provider's keys/URL without verifying provider identity. Configuration-first startup can mix unrelated provider/credentials/endpoint; fix with one captured connection snapshot, test mismatched saved credentials and endpoint explicitly. Model switching must reuse that live snapshot and allow registered custom models; preserve all keys and avoid leaking credentials on reconstruction. Then continue remaining shared budget paths (TUI still allocates separate TemDOS/JIT trackers and drops Perpetuum usage), typed durable goal/evidence contracts and broader feature acceptance/A/B. Preserve pending D01/D02 defaults; no new automatic pursuit or host privilege changes.
...
After pushing checkpoint 34, inspect CI and fix the TUI connection-identity issue above. Keep the following boundaries visible; the browser checkpoint is not complete product acceptance.
