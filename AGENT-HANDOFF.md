...
Checkpoint **37** fixes classifier usage lost after JSON parsing failure. Read `SHARED-ACCOUNTING-IMPLEMENTATION.md`. Runtime gives classify_message a MeteredProvider over a local BudgetTracker child; provider settlement precedes parsing, local turn counters include failed parsing, and success is not re-recorded. Timeout/cancel unknown outcomes settle once. `BudgetSnapshot.recorded_calls` is saturating logical outcome accounting, not HTTP retry attempts. Raw classifier-response debug text is removed.

Validation: **787 agent unit +15 runtime integration tests**, including valid/malformed classifier JSON exactly2calls/20in/40out; token cancellation and task deadline record one unknown then reject subsequent limited work before another invocation. Actual TUI PTY suite passes at $0.0002 after both first-turn responses are charged; switching an exhausted session makes zero more HTTP calls. Logs: `implementation-classifier-accounting-final-tests.log`, `implementation-classifier-accounting-pty.log`. Full workspace/all-feature/all-target lint passed1m47s (`implementation-classifier-accounting-workspace-clippy.log`) and cleaned2.6GiB. Dependency proc-macro-error2 2.0.1 still emits its future-compatibility notice, not a lint failure. No Cargo remains active. No paid call was made.

Previous pushes **36 `33e4cee`**, CI34268787507, and **35 `76350ea`**, CI34268068127, pending. **34 `e5dc5d9`**, CI34267201313, **33 `a424b74`**, CI34265542264, **32 `816a75d`**, CI34263918328, **31 `df4e6bd`**, CI34263300717, and **30 `9df96c9`**, CI34261794031, are fully green. **29 `06d9bbd`**, CI34261489651, remains recorded Windows failure (Hive stale-readiness race repaired in32).

Next concrete task: **root CLI/server budget continuity and automatic Hive accounting**. `src/main.rs` has25 AgentRuntime::with_limits constructors, most with_durable_execution but no owning budget. Only two inner initial runtime_budget variables and the initial CLI Perpetuum rebuild currently share one. Allocate one process-command owning tracker after config load (before match), remove shadowing initial allocations and bind every reconstruction. Clone through channel-loop/message-worker closures; automatic Hive worker closure needs a child of the parent tracker. Its maybe_decompose callback currently uses raw provider and widens token counts only after u32 addition; meter it and widen operands before addition. Preserve model/provider choices and do not double-charge returned worker totals. Add real CLI model/tool-reload budget-continuity fixtures; inspect all-feature compilation for moved captures. This is in-process continuity, not restart-persistent or principal-global policy. Then continue durable goal/evidence contracts and broad held-out A/B. Preserve pending D01/D02 defaults.
...
After pushing checkpoint 37, inspect CI and complete root budget continuity above. Keep the following boundaries visible; the browser checkpoint is not complete product acceptance.
