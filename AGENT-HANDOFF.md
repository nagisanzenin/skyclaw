...
Checkpoint **39** centralizes connection resolution in core and uses it for Start/Chat/TUI. Read `CONNECTION-RESOLUTION-IMPLEMENTATION.md`. Configured provider/endpoint/keys/model stay together; root startup no longer rereads unrelated saved keys. Config-only pools and keyless Codex work; headers remain scoped. Setup messages say configured rather than falsely verified, and validation debug output omits raw endpoint URLs.

Validation: **260core(one ignored),50TUI,79root minimal tests** pass. Real CLI/TUI fixtures with wrong saved endpoint traps pass alongside budget continuity. Real server config-only key pool selects the intended provider/model despite foreign invalid saved endpoint, preserves saved file, makes zero provider requests and exits0 on SIGTERM. Logs `implementation-shared-connection-{tests,root-tests,cli-smoke,tui-pty,server-smoke}.log`. Initial workspace lint failed two needless borrows and cleaned2.8GiB; fixed. Final workspace/all-feature/all-target lint passed2m09s and cleaned1.2GiB (`implementation-shared-connection-final-clippy.log`); only the existing proc-macro-error2 dependency future-compatibility notice remains. No Cargo is active. No paid calls.

Previous pushes **38 `b63fcc5`**, CI34271019124, **37 `2d5f122`**, CI34269989662, both fully green. **36 `33e4cee`**, CI34268787507, **35 `76350ea`**, CI34268068127, **34 `e5dc5d9`**, CI34267201313, **33 `a424b74`**, CI34265542264, **32 `816a75d`**, CI34263918328, **31 `df4e6bd`**, CI34263300717, and **30 `9df96c9`**, CI34261794031, fully green. **29 `06d9bbd`**, CI34261489651, remains recorded Windows failure repaired in32.

Next concrete task: **shared runtime feature policy, especially explicit memory disablement**. `with_engram_config` and `with_blueprint_notice` occur only on two root server startups; CLI/TUI and rebuilds silently use constructor defaults. Introduce a typed common RuntimePolicy from AgentConfig/EngramConfig, applied by every root/TUI constructor and JIT worker. Preserve v2/self-audit/parallel/blueprint/Engram choices through provider/model/tool changes, using one captured policy rather than copying five calls25times. SpawnSwarmContext should carry that policy for descendants. Add actual-entrypoint fixtures seeded with a permanent-memory sentinel and Engram disabled: no injection/curation despite a model reconstruction, with enabled case proving the fixture can detect injection. Do not conflate this with full AppServices: consciousness, Perpetuum, EigenTune and bound tool-provider handles still need a common resource/factory contract. Root post-start credential/model reload remains separate; CLI `/model` parity is not implemented. Continue durable goal/evidence and broad held-out A/B afterward. D01/D02 defaults remain unchanged.
...
After pushing checkpoint 39, inspect CI and preserve runtime feature/memory policy above. Keep the following boundaries visible; the browser checkpoint is not complete product acceptance.
