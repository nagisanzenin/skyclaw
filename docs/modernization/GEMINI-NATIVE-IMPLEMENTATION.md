...
## Checkpoint 32 integration correction

A review of the production stream collector found that the provider still inherited the non-streaming `complete_with_observer` default, and native state needed to share its terminal chunk with the completion marker. The observer now calls the shared collector; final native state, stop and usage arrive together. Provisional chunks omit an unknown provider response ID instead of inventing one that could change later. The two-turn HTTP fixture now exercises `complete_with_observer` directly and releases the server remainder only after the observer receives public Unicode text. Combined provider/Hive tests and scoped all-target lint pass (`implementation-observer-hive-integration-tests.log`, `implementation-observer-hive-final-clippy.log`). This extends checkpoint 31’s adapter-level evidence to the actual runtime observer path.
