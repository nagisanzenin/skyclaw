...
## Checkpoint 30 — selectable modern model IDs

The common registry now exposes the already source-dated Astra/5.6/5.5, Claude 5/Fable, Gemini 3.8 and Grok 4.6 IDs to CLI and TUI menus. Codex subscription receives its own native-Responses model suggestions instead of an empty list. `/model` describes these as suggestions with account-dependent access. Existing saved defaults and the previous first menu selections are preserved; adding a catalog entry does not automatically select a more expensive model. Existing legacy IDs remain selectable, and custom model registration remains the explicit extension path.

Validation: the core package passed 254 tests, one ignored test and one doctest (`implementation-model-menu-core-tests.log`); menu ordering was then adjusted to preserve prior preselection. Formatting and diff checks pass. No paid account call was made. Static suggestions do not prove entitlement, provider quota or live acceptance; current model metadata still needs capability-knownness and account discovery work. Native Responses and Anthropic replay are implemented in checkpoints 22/26, superseding the corresponding earlier remaining-work items above. Gemini streaming is still outstanding.


## Checkpoint 34 — image capability evidence and custom overrides

The catalog records `image_input: true|false` only with a per-model primary source and checked date. Absence means unknown. CLI model badges and TUI onboarding distinguish verified vision, explicit text-only and unknown image support. A similar model name, arbitrary namespace or a proxy route is not evidence. The built-in Codex/Z.ai subscription aliases resolve their vendor capability, but this does not prove account entitlement.

Source check on 2026-09-09: exact OpenAI model input-modalities entries, including [o3-mini](https://developers.openai.com/api/docs/models/o3-mini), establish that o3-mini does **not** support image input; the old agent test asserted the opposite. [Claude's current model overview](https://platform.claude.com/docs/en/models/overview), [Grok 4.6](https://docs.x.ai/developers/models/grok-4.6), [Gemini 3.8 Flash](https://ai.google.dev/gemini-api/docs/models/gemini-3.8-flash) and [GLM 5.3 Flash](https://docs.z.ai/guides/vlm/glm-5.3-flash) support their explicit image flags. Individual OpenAI page links are stored with each verified catalog row. Failed legacy-page retrievals and models absent from the checked current overview remain unknown; neither similarity nor redirects are treated as verification.

Custom model TOML gains optional `image_input`. `/addmodel <name> context:<n> output:<n> vision:true|false|unknown` sets, rejects or clears image input; omission preserves an existing custom setting. A registered custom model with omitted capability stays unknown even if its name equals a catalog entry. Old TOML remains readable and does not acquire an invented capability. A runtime captures the provider/model decision at construction, so changing registration takes effect in a new runtime. `/listmodels` also distinguishes unspecified zero prices from verified zero rates and subscription quota-unknown billing.

Explicit true/false controls image forwarding. To preserve existing proxy compatibility, unknown capability uses the separately named legacy forwarding policy and logs that it is unverified. It never earns a vision badge. If that policy excludes an image, notices describe the policy/unknownness instead of falsely asserting vendor incapability. This is an intermediate migration contract: unknown forwarding still contains historical family heuristics and is not a verified feature. The current resolution lacks endpoint/version identity and a full immutable ResolvedModel; a custom base URL impersonating a first-party provider needs explicit custom registration to override metadata. TUI custom-model switching, all media types, account discovery and a removal plan for legacy heuristics remain unfinished.

Validation results are recorded with checkpoint 34 in IMPLEMENTATION-STATUS after execution. No paid provider was invoked for this change.
