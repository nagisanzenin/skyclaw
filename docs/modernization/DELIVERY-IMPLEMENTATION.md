...
Implementation contract, not a claim that the outbox is shipped. The current channel trait returns `Result<()>` from `send_message`; it provides neither a platform message receipt nor a reconciliation/idempotency capability. A generic transport error therefore cannot establish whether the platform accepted the message.
