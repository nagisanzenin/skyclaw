...
## Onboarding gateway correction

The new actual-container smoke exposed a second failure after fixing the missing runtime library: first-run onboarding never constructed SkyGate, so `/health` could not respond. SkyGate now shares the optional runtime used by onboarding and model switching. Liveness is always available; readiness is 503 until a runtime exists and 200 afterward. Dashboard requests return explicit onboarding responses rather than retaining an obsolete runtime. The gateway receives the root cancellation token for graceful server shutdown.

The image's default command now explicitly binds `0.0.0.0` inside the container via the new `start --host` option. General CLI defaults remain loopback. Compose publishes on host loopback. A real binary smoke checks two no-key starts using the same private profile, health/readiness, profile reopening, blueprint parser logs and clean SIGTERM. CI runs that smoke as well as the Docker volume-replacement smoke. Neither readiness nor liveness proves provider reachability or subscription quota.

Four built-in Prowl blueprints had no `trigger_patterns` and failed the real agent parser despite superficial frontmatter tests. They now provide trigger phrases, and a root integration test parses every shipped blueprint with the production schema. The login procedure uses `restore_web_session(service=...)` and directs OTK setup to the actual `/login` host command. It no longer instructs the model to invoke an unsupported `authenticate(method="otk")`. The comparison procedure requires confirmed isolated browser contexts before parallel browsing rather than asserting they always exist.
