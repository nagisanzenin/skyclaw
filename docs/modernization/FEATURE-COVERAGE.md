...
| Witness objective/identity/accounting | `WITNESS-GOAL-BINDING-IMPLEMENTATION.md`; checkpoint47 actual two-turn CLI and malformed/error/cancel tests | Command authority repaired in checkpoint48 (WITNESS-AUTHORITY-IMPLEMENTATION.md), with real User denial/Admin control. Frozen scoped criteria repaired49 (GOAL-CRITERIA-IMPLEMENTATION.md); Three-valued logic/seal integrity repaired50 (WITNESS-KNOWNNESS-IMPLEMENTATION.md); Actual typed observations/declared assessment integrated51 (GOAL-ASSESSMENT-IMPLEMENTATION.md); raw inspected evidence, coverage and configured tier evidence remain open. |
