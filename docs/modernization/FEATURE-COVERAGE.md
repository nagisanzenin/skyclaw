...
## Implementation evidence through checkpoint56
...
| Engram persistence, identity and numeric policy | `ENGRAM-NUMERIC-IMPLEMENTATION.md`, `TOOL-IDENTITY-IMPLEMENTATION.md`, `ENGRAM-PERSISTENCE-IMPLEMENTATION.md`; checkpoints42–44, real scoped SQLite persistence/deletion and Markdown unsupported acceptance; checkpoint56 fixes acknowledged-write visibility (MARKDOWN-WRITE-VISIBILITY.md) | EMA/provenance/cadence, channel-qualified principals and promised Markdown Engram fallback remain open. |
| Witness objective/identity/accounting | `WITNESS-GOAL-BINDING-IMPLEMENTATION.md`; checkpoint47 actual two-turn CLI and malformed/error/cancel tests | Command authority repaired in checkpoint48 (WITNESS-AUTHORITY-IMPLEMENTATION.md), with real User denial/Admin control. Frozen scoped criteria repaired49 (GOAL-CRITERIA-IMPLEMENTATION.md); Three-valued logic/seal integrity repaired50 (WITNESS-KNOWNNESS-IMPLEMENTATION.md); Actual typed observations/declared assessment integrated51 (GOAL-ASSESSMENT-IMPLEMENTATION.md); Scoped inspected file bytes integrated53 (WITNESS-EVIDENCE-IMPLEMENTATION.md); Provider judgment parsing is bounded in54 (WITNESS-VERIFIER-CONTRACT.md); explicit bounded-call configured tiers use current model/owner in55 (WITNESS-CONFIGURED-PROVIDER.md). Non-file sources, full coverage, USD reservations and subscription account quota remain open. |
| Durable goals and evidence | `GOAL-LEDGER-IMPLEMENTATION.md`; checkpoint46 active admission/tool/return journal, scoped readonly CLI/TUI and actual persistence/hash/restart tests | Frozen criteria integrated49; actual declared assessment consumer integrated51; task queue leases/recovery, child lineage and D02 continuation remain open; no success-from-prose. |
...
| 0.5 | Raised limits | core/types/config.rs; agent/runtime.rs | Larger round/time caps are policy, not robustness. Checkpoint52 separates actual tool cancellation from journal I/O (CANCELLATION-PHASE-VALIDATION.md); Windows acceptance pending, production admission remains outside cancellation/deadline select. Shared admission/bounded outputs P07/P10. |
