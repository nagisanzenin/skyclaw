...
**Real code-level proof — Gemini 3 Flash, autonomously, in 40 seconds:**

| Stage | Result |
|-------|:---:|
| LLM generates `format_bytes` function + 5 unit tests | OK |
| `cargo check` | OK |
| `cargo clippy --all-targets -- -D warnings` | OK |
| `cargo test` | **6 passed, 0 failed** |

Cost: ~$0.001. Sandbox: isolated tempdir. The model generated correct Rust on the first try, including a doc comment, a loop-based unit progression through B/KB/MB/GB/TB/PB/EB, the `{:.1}` format specifier for decimal places, the edge case for bytes < 1024, and 5 distinct test cases (zero, small, kilobytes, megabytes, large). It also preserved an existing `marker()` function exactly. This is not skill markdown — it is real production-grade Rust generated by a $0.001 cheap-tier model and verified by the deterministic harness.
...
**Enabled by default in v4.7.0.** Toggle with `/cambium on` and `/cambium off`. 2,281 tests passing. The full architecture and the empirical proof are in the [research paper](tems_lab/cambium/CAMBIUM_RESEARCH_PAPER.md).

[Research paper →](tems_lab/cambium/CAMBIUM_RESEARCH_PAPER.md) · [Theory (10 first principles) →](docs/lab/cambium/THEORY.md) · [Implementation plan →](docs/lab/cambium/IMPLEMENTATION_PLAN.md) · [Protected zones →](docs/lab/cambium/PROTECTED_ZONES.md)
