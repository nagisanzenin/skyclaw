...
<p align="center">
  ...
  <br>
  <sub>Heartwood (immutable kernel) · Cambium (growth layer: tools, skills, cores) · Bark (runtime surface) · Rings (GrowthSession history). Toggle with <code>/cambium on</code> · <code>/cambium off</code></sub>
  <br>
  <sub><strong>Real code-level proof:</strong> Gemini 3 Flash autonomously wrote a complete <code>format_bytes</code> function with 5 unit tests in 40s for ~$0.001. <code>cargo check</code> ✓ <code>cargo clippy -D warnings</code> ✓ <code>cargo test</code> ✓ — all 6 tests passed. Not skill markdown — actual Rust generated by an LLM and verified by the deterministic harness.</sub>
</p>
